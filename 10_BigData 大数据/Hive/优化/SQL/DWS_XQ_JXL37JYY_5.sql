--年度宽一应收日期+应收保费
--3760211
DROP TABLE ZTEST_JXL_37J_CONT_WYM1YS PURGE;

CREATE TABLE ZTEST_JXL_37J_CONT_WYM1YS AS
     SELECT
          P.CONTNO,
          P.POLNO,
          P.TJMONTH,
          ...
          END       PREM
     FROM
          DWS_XQ_JXL37JYY_TMP_5_1      P
          LEFT JOIN(
               SELECT
                    J.CONTNO,
                    J.POLNO,
                    DATE_FORMAT(ADD_MONTHS(J.LASTPAYTODATE, 13), 'yyyyMM')           LASTPAYTODATE,
          ...
          LEFT JOIN(
               SELECT
                    J.CONTNO,
                    J.POLNO,
                    DATE_FORMAT(ADD_MONTHS(J.CURPAYTODATE, 1), 'yyyyMM')             CURPAYTODATE,
                    ROUND(DATEDIFF(MAX(J.CURPAYTODATE), MAX(J.LASTPAYTODATE)) / 365) ND
               FROM
                    ODS.O_LIS_LJAPAYPERSON J
               WHERE
